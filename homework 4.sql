...
WITH Expensive_Cities AS (

select CityID,CityName,so.SalespersonPersonID PersonID
...
INNER JOIN (SELECT TOP(3) StockItemID,StockItemName, MAX(UnitPrice) max_unit
			FROM Warehouse.StockItems
			GROUP BY StockItemID,StockItemName
			ORDER BY max_unit desc) e ON e.StockItemID = sol.StockItemID
GROUP BY CityID,CityName,so.SalespersonPersonID

)

SELECT sub.CityID,sub.CityName,ap.FullName
