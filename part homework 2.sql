...
select StockItemName as [Товар], 
	   ...
	   IIF(DATEPART(mm,ConfirmedDeliveryTime) between 1 and 4, 'first third', IIF(DATEPART(mm,ConfirmedDeliveryTime) between 5 and 8,'second third',IIF(DATEPART(mm,ConfirmedDeliveryTime) between 9 and 12,'third third',''))) as [Треть года]
...
Order BY [Квартал продажи], [Треть года], [Дата продажи] 
 ...
 select StockItemName as [Название товара], OrderID as [Номер заказа], DeliveryMethodName as [Метод доставки], SupplierName as [Имя поставщика], ConfirmedReceivedBy as [Контактное лицо, принявшее заказ]
...
select c1.CustomerID as [ID покупателя], CustomerName as [Покупатель], c1.PhoneNumber as [Телефонный номер], StockItemName as [Название товара], i2.Quantity as [Количество], i2.ExtendedPrice as [Финальная цена]
